...
export default function ReportWriter() {
  ...
  const handleGenerate = async () => {
    if (!topic.trim() || !context.trim() || !incidentDate) {
      toast({
    ...
    router.push("/tools/report/loading")
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900">
      ...
      <header className="relative z-10 flex items-center gap-4 p-6 pt-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Report Writer</h1>
            <p className="text-sm text-gray-400">Generate professional reports with ease</p>
          </div>
        </div>
      </header>
          ...
          <Card className="p-8 mb-8 bg-gray-800/30 backdrop-blur-xl border border-gray-700/50 shadow-2xl rounded-3xl">
            <div className="space-y-8">
              <div>
                ...
                <Input
                  id="topic"
                  placeholder="Enter report title or subject..."
                  value={topic}
                  onChange={(e) => setTopic(e.target.value)}
                  className="bg-gray-800/50 border border-gray-700/50 rounded-2xl px-6 py-4 text-white placeholder:text-gray-400 focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 text-lg"
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  id="context"
                  placeholder="Describe what happened and how it happened..."
                  value={context}
                  onChange={(e) => setContext(e.target.value)}
                  className="bg-gray-800/50 border border-gray-700/50 rounded-2xl px-6 py-4 text-white placeholder:text-gray-400 focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 text-base min-h-[120px]"
                />
              </div>

              <div>
                <Label htmlFor="incidentDate" className="text-sm font-medium text-gray-300 mb-3 block">
                  Date of Incident *
                </Label>
                <Input
                  id="incidentDate"
                  ...
                  className="bg-gray-800/50 border border-gray-700/50 rounded-2xl px-6 py-4 text-white focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 text-base"
                />
              </div>
              ...
              <Button
                ...
                disabled={!topic.trim() || !context.trim() || !incidentDate}
